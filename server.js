...
http.createServer(function(request, response) {
    ...
    console.log("type: ", typeof receivedData)
      ...
      if (request.method == "POST") {
        ...
        if (exists.existsSync('songs/' + dataObj.text + '.txt')){
        let buffer = fs.readFileSync('songs/' + dataObj.text + '.txt', 'utf8')
        var returnObj = {}
        returnObj.wordArray = buffer.toString(); //object to return to client
      ...
      if (request.method == "GET") {
        ...
        fs.readFile(filePath, function(err, data) {
          if (err) { //report error to console
            console.log("ERROR: " + JSON.stringify(err)) //respond with not found 404 to client
